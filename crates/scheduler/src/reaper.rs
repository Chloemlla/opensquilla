...
impl SessionReaper {
    ...
    pub fn with_session_ttl(mut self, hours: u64) -> Self {
        self.session_ttl_hours = hours;
        self
    }
    pub fn with_keep_executions(mut self, keep: u64) -> Self {
        self.keep_executions = keep;
        self
    }
    pub fn with_completed_job_retention(mut self, days: u64) -> Self {
        self.completed_job_retention_days = days;
        self
    }
    ...
    pub async fn run_cycle(&self) -> ReaperResult {
        ...
        if let Ok(count) = store.cleanup_executions(self.keep_executions) {
            ...
            if count > 0 {
                debug!("Reaper: cleaned up {} old execution records", count);
            }
        } else {
            warn!("Reaper: failed to cleanup executions");
        }

        if let Ok(count) = store.cleanup_completed_jobs(self.completed_job_retention_days) {
            cleaned_jobs = count;
            if count > 0 {
                debug!("Reaper: cleaned up {} completed jobs", count);
            }
        } else {
            warn!("Reaper: failed to cleanup completed jobs");
        }
        ...
        if cleaned_executions > 0 || cleaned_jobs > 0 {
            info!(
                "Reaper: {} executions, {} jobs cleaned in {}ms",
                cleaned_executions, cleaned_jobs, duration_ms
            );
        }
    ...
    fn auto_disable_stale_failed_jobs(
        &self,
        store: &JobStore,
        ...
        for job in failed_jobs {
            if let Some(last_run) = job.last_run_at {
                if (now - last_run).num_hours() as u64 >= self.session_ttl_hours {
                    ...
                    info!(
                        "Reaper: auto-disabled job '{}' after {} hours in Failed status",
                        job.name, self.session_ttl_hours
                    );
                }
            }
        }
    ...
    pub fn cycles_run(&self) -> u64 {
        self.cycles_run.load(std::sync::atomic::Ordering::SeqCst)
    }
}
